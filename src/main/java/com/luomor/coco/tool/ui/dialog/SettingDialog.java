...
import com.luomor.coco.tool.service.HttpMsgSender;
import com.luomor.coco.tool.ui.form.func.*;
        ...
        dbFilePathSaveButton.addActionListener(e -> {
            try {
                ...
                if (StringUtils.isNotBlank(dbFilePath)) {
                    FileUtil.copy(dbFilePathBefore + File.separator + "CocoTool.db", dbFilePath, false);
                }
                ...
                App.config.save();
                JOptionPane.showMessageDialog(contentPane, "保存成功！\n\n需要重启CocoTool生效", "成功", JOptionPane.INFORMATION_MESSAGE);
                JOptionPane.showMessageDialog(contentPane, "CocoTool即将关闭！\n\n关闭后需要手动再次打开", "CocoTool即将关闭", JOptionPane.INFORMATION_MESSAGE);
                System.exit(0);
            } catch (Exception e1) {
